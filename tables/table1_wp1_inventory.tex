\begin{table}
\caption{Frozen WP1 data inventory. All artifacts carry SHA-256 checksums in \texttt{provenance/wp1\_manifest.json}.}
\label{tab:wp1}
\centering
\begin{tabular}{lrrr}
\hline\hline
Dataset & Selection & $N$ & Note \\
\hline
Gaia DR3 narrow box & $l$ 77--83, $b$ $-$1.5--4 & 245,843 & WP2 input \\
Gaia DR3 wide box & $l$ 72--88, $b$ $-$5--8 & 3,133,326 & WP6 traceback \\
2MASS PSC matches & official best-neighbour & 228,654 & 93.0\% \\
\quad complete $JHK_s$ & finite uncertainties & 203,646 & 82.8\% \\
\quad ph\_qual AAA & highest quality & 170,757 & 69.5\% \\
Wright+15 census & matched to Gaia DR3 & 167 / 167 & 100\% \\
Spectroscopic anchors & canonical, unique IDs & 252 & 540 evidence rows \\
ATNF pulsars (wide) & SN markers & 80 & incl.\ PSR J2032+4127 \\
Green SNRs (wide) & SN markers & 9 & incl.\ G78.2+2.1 \\
\hline
\end{tabular}
\end{table}
