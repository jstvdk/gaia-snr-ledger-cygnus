...
### 18.8 `repair_v7` executed and accepted (2026-07-29)

The chain was re-run with the extended truth-side f_bin applied over the **full**
mass range — the WP5 base grid (`--fbin-model extended`, 162 nodes) *and* the
WP6 mass extension (162 nodes, 20–115 M☉). `repair_v6` keeps the unsuffixed
filenames throughout, so every hash already recorded against it stays valid;
**V1 re-run confirms 5/5 accepted artifacts byte-identical**.

| quantity | `repair_v6` | `repair_v7` |
|---|---:|---:|
| WP5 normalization `k` | — | **+0.54% ± 0.25%** |
| closure ratio, CygOB2-A | 0.894 | **0.865** |
| closure ratio, CygOB2-B | 1.106 | **1.068** |
| closure ratio, CygOB2-C | 1.405 | **1.360** |
| grid median (54 cells) | 1.105 | **1.067** |
| cells consistent with unity at 68% | 6/54 | **9/54** |
| closing α (grid median) | 2.230 | **2.254** |
| cells inside the carried branch grid | 17/18 | **18/18** |

**The `k` shift matched the out-of-sample prediction exactly** — the discriminator
gave +0.54% ± 0.19% from three nodes in ~3 minutes; the full 54-cell re-run gave
+0.54% ± 0.25%. The closure prediction was slightly less sharp: §18.7 projected a
grid median of ~1.074 against a measured 1.067, i.e. the movement was **−3.4%
where −2.3% was estimated**. The estimate came from a single segment-weight
argument and is recorded as approximate, not as a miss of the acceptance test —
no threshold was defined on it.

The joint fit re-passed **G3** with `A_or_C_regressions: []` and 40/54 branches,
identical to `repair_v6`.

**One provenance gap, found and closed.** The injection driver did not record
which f_bin model it used, because `--fbin-model` was added in the same commit
that launched the run. The model was verified from the outputs instead — realized
f_bin against the pre-registered curve gives **χ² = 41.2 on 45 masses, p = 0.63**,
mean z = +0.13, with `repair_v6` flat at 0.41 as control. Both drivers now record
the model directly, and the closure test **reads it from the extension's own
provenance and refuses to run on a base/extension mismatch** — the two are
generated by different drivers and would otherwise concatenate silently.

### 18.9 Issue #15's correction is now internal — do not apply it twice

`repair_v7`'s f_bin is mass-dependent over the whole range, so above 8 M☉ it
**contains** the model issue #15 tested as its treatment arm:

| M / M☉ | `repair_v6` constant | issue #15 treatment | `repair_v7` |
|---:|---:|---:|---:|
| 8 | 0.40 | 0.40 | **0.55** |
| 12 | 0.40 | 0.575 | **0.638** |
| 16 and above | 0.40 | 0.70 | **0.70** |

The two agree at and above 16 M☉ and `repair_v7` is **strictly higher at 12
masses in between**, because its below-8 rise has to join continuously at the
threshold. `repair_v7` therefore carries slightly *more* multiplicity above
8 M☉ than issue #15's treatment arm did.

**Consequence: the grid-median reduction of 0.0038 that §18.6 directed be carried
as a systematic must NOT be applied to any `repair_v7` ratio.** It is already
inside the response matrices. It remains applicable to the `repair_v6` numbers,
which were generated under a constant 0.40.

What is subsumed is the **numerical correction**, not the **finding**. Issue #15's
M1/M2/M3 scores were a paired differential measurement of how much multiplicity
above 8 M☉ explains the excess, and the answer — 3.7%, so not the explanation —
stands independently of which chain version is reported.

At 0.36% of the reported ratio this would not have changed a conclusion. It is
recorded because a bookkeeping error that happens to be small is still a
bookkeeping error. Verified programmatically in
[wp6_multiplicity_subsumption.json](provenance/wp6_multiplicity_subsumption.json).
